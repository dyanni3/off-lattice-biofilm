...
int max_num_cells=int((700/(2*cell_radius))*(700/(2*cell_radius))*100); //should adjust so that if you packed spheres effeciently into the volume they would just fill up the volume. optimal packing of spheres is .74
...
PVector gravity=new PVector(0,18,0);
int counter=0;
int global_size=700;
...
int growing=1;
  ...
  void init(){
      num_cells=0;
      bugs.bacteria.clear();
      for(int i=0;i<max_num_cells/10;i++){
